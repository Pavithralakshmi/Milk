        ...
        <table id="datatable1" class="table diagnosis_list">
            <thead>
                <tr>                                    
                    ...
                    <th style="text-align: left">Total Liter Per Day</th>
                </tr>
            </thead>
            <tbody class="ui-sortable" >
                <?php
                $i = 1;
//                $sql = "select *  from cowsell  where (`date` BETWEEN '$FromToBalanceResultFrom' AND '$FromToBalanceResultTo')";
                $sql = "SELECT type,cowtype,breedtype,name,date,sum(`total`)as total_sum1  from `dailymilk` where (`date` BETWEEN '$From' AND '$To') group by date,breedtype";
                ...
                while ($row_main = mysqli_fetch_assoc($result)) {
                    ...
                    <tr>
                        ...
                        <td style="text-align: center"><?php
                            if ($row['date'] == "") {
                                echo date('d-m-Y', strtotime($row_main['date']));
                            } else {
                                echo date('d-m-Y', strtotime($row_main['date1']));
                            }
                            ?></td>                   
                        <td style="text-align: left"><?php echo $row_main['type'] . "-" . $get_cowtype[$row_main['name']] . $get_productname[$row_main['breedtype']]; ?></td>
                        <td style="text-align: center"><?php echo $row_main['total_sum1']; ?></td>
                    </tr> 
                    <?php
                    $i++;
                }
...
<html lang="en">
    <head>
        <title>Milk Management-Daily Milking Report</title>
                        ...
                        <div class=" col-md-12 col-sm-12">
                            <div class="card">
                                <div class="card-head style-primary">
                                    <header>Daily Milking Report</header>
                                </div>
                                <div class="card-body">
