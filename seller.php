...
if (isset($_POST['save'])) {
    ...
    $cdate=date('y/m/d');
    if ($id) {
        $check = "select * from `seller` where  `phoneno`='$phoneno'AND id !='$id'";
        $res = mysqli_query($mysqli, $check);
        if (mysqli_num_rows($res)) {
            $msg = 1;
        } else {
            $sql = "UPDATE `seller` SET `date`='$timestamp',`name`='$name',`phoneno`='$phoneno',`address`='$address',`remark`='$remark', `mby` = CONCAT (`mby`,'|','$user'), `mdate` = CONCAT (`mdate`,'|','$datetime'), `mip` = CONCAT (`mip`,'|','$_SERVER[REMOTE_ADDR]') WHERE id='$id'";
            $result = mysqli_query($mysqli, $sql) or trigger_error("Query Failed! SQL: $sql - Error: " . mysqli_error($mysqli));
            header("Location: seller.php?msg=3");
            exit;
        }
    } else {
        $check = "select * from `seller` where `phoneno`='$phoneno'";
        $res = mysqli_query($mysqli, $check);
        if (mysqli_num_rows($res)) {
            $msg = 1;
        } else {
            $sql = "INSERT INTO `seller`(`date`, `name`,`phoneno`, `address`, `remark`,`cby`, `cdate`, `cip`) VALUES ('$timestamp','$name','$phoneno', '$address', '$remark','$user','$cdate','$_SERVER[REMOTE_ADDR]')";
            $result = mysqli_query($mysqli, $sql) or trigger_error("Query Failed! SQL: $sql - Error: " . mysqli_error($mysqli));
            header("Location: seller.php?msg=2");
            exit;
        }
    }
}
...
if (isset($_GET['operation']) && $_GET['operation'] == 'delete') {
    $id_val = $_GET['id'];
    $sql = "DELETE FROM `seller` where `id` = '$id_val'";
//    echo $sql;exit;     
    $result = mysqli_query($mysqli, $sql);
    ...
    header('Location: seller.php?msg=' . $msg);
    exit;
}
?>
...
<html lang="en">
    <head>
        <title>Milk Management- Seller Registration</title>
        ...
        <div id="base">
            ...
            <div id="content">
                <section>
                                        ...
                                        <form class="form  form-validate" role="form" method="POST">
                                            <div class="row">
                                                <div class="col-sm-6 form-group">
                                                    <input type="text" class="form-control" required="true" maxlength="35" minlength="3" name="name" tabindex="1" value="<?php echo $name; ?>">
                                                    <label for="name">&nbsp; &nbsp;Selller Name<sup style="color:red;">*</sup></label>
                                                </div>
                                                <div class="col-sm-6 form-group ">
                                            ...
                                            <div class="row">
                                                <div class="col-sm-6 form-group">
                                                    <input type="number" class="form-control" pattern="[6789][0-9]{9}" required="true"  name="phoneno" tabindex="1" value="<?php echo $phoneno; ?>">
                                                            ...
                                                            <tbody class="ui-sortable" >
                                                                ...
                                                                while ($row = mysqli_fetch_assoc($result)) {
                                                                    ...
                                                                    <tr  id="<?php echo $row['id']; ?>"  >
                                                                        <td><?php echo $i; ?></td>
                                                                        <td class="text-left">   
                                                                            <a href="seller.php?id=<?php echo $id; ?>"><button type="button" class="btn ink-reaction btn-floating-action btn-info" data-toggle="tooltip" data-placement="top" data-original-title="Edit row"><i class="fa fa-pencil"></i></button></a>
                                                                            <?php
                                                                            $NumType = 0;
                                                                            $sql_type = "select `id` from `inward` where `phoneno` = '$row[phoneno]'";
                                                                            $result_type = mysqli_query($mysqli, $sql_type);
                                                                            if (!mysqli_num_rows($result_type)) {
                                                                                ?> 
                                                                                <a href="seller.php?id=<?php echo $id; ?>&operation=delete"><button  style="margin-bottom: 3px;" type="button" class="btn ink-reaction btn-floating-action btn-danger"    data-toggle="tooltip" onclick="return confirm('Are you sure to delete?')" data-placement="top" data-original-title="Delete row"><i class="fa fa-trash"></i></button></a>
                                                                                    <?php } ?>
                                                                        </td>
                    ...
                    <script>
<?php if ($msg == '2') { ?>
                            Command: toastr["success"]("Seller Details Registered  Sucesssfully", "Sucesss")
<?php } elseif ($msg == '1') {
    ?>
                            Command: toastr["error"]("Same Seller already exist", "Error")
<?php } elseif ($msg == '3') { ?>
