...
if (isset($_GET['id'])) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $date12 = $row['date12'];
        $date12 = new DateTime($date12);
        $timestamp12 = $date12->format('d-m-Y'); // 31.07.2012   
...
if (isset($_POST['save'])) {
    ...
    $date12 = $_POST['date12'];
    $date12 = new DateTime($date12);
    $timestamp12 = $date12->format('Y-m-d'); // 31.07.2012 
...
//    echo $address;EXIT;
    ...
    if ($id) {
        ...
        $sql = "UPDATE `exp` SET `inwardno`='$inwardno',`expencestype`='$expencestype',`sellername`='$sellername',`sellername1`='$sellername1',`phoneno`='$phoneno',`address`='$address',`expences`='$expences',`rate`='$rate',`total_unit`='$total_unit',`total_amount`=$total_amount,`date`='$timestamp',`date12`='$timestamp12',`unit`='$unit',`paid`='$paid',`balance`='$balance',`remark`='$remark' WHERE id='$id'";
        $result = mysqli_query($mysqli, $sql) or trigger_error("Query Failed! SQL: $sql - Error: " . mysqli_error($mysqli));
        header("Location: exp.php?msg=3");
        exit;
    } else {
//        $sql = "INSERT INTO `ex`(`expences`, `date`,`sellername`,`phoneno`,`address`,`total_unit`,`total_amount`, `unit`,`rate`,`remark`) VALUES('$expences','$timestamp','$buyername','$phoneno','$address','$total_unit','$total_amount','$unit','$rate','$remark')";
        $sql = "INSERT INTO `exp`(`inwardno`, `expencestype`,`sellername`,`sellername1`,`phoneno`,`address`,`expences`,`rate`,`total_unit`,`total_amount`,`date`,`date12`,`unit`,`paid`,`balance`,`remark`) VALUES ('$inwardno','$expencestype','$sellername','$sellername1','$phoneno','$address','$expences','$rate','$total_unit','$total_amount','$timestamp','$timestamp12','$unit','$paid','$balance','$remark')";
//        echo $sql;exit;
        $result = mysqli_query($mysqli, $sql) or trigger_error("Query Failed! SQL: $sql - Error: " . mysqli_error($mysqli));
        header("Location: exp.php?msg=2");
        exit;
    }
}
if (isset($_GET['operation']) && $_GET['operation'] == 'delete') {
    ...
    header('Location: exp.php?msg=' . $msg);
}
...
//                echo $total_amount1;exit;
                            ...
                            <tr  id="<?php echo $row['id']; ?>"  >
                                <td><?php echo $i; ?></td>
        <!--                                <td class="text-left">   
                                    <a href="exp.php?id=<?php echo $id; ?>"><button type="button" class="btn ink-reaction btn-floating-action btn-info" data-toggle="tooltip" data-placement="top" data-original-title="Edit row"><i class="fa fa-pencil"></i></button></a>
                                    <a href="exp.php?id=<?php echo $id; ?>&operation=delete"><button  style="margin-bottom: 3px;" type="button" class="btn ink-reaction btn-floating-action btn-danger"    data-toggle="tooltip" onclick="return confirm('Are you sure to delete?')" data-placement="top" data-original-title="Delete row"><i class="fa fa-trash"></i></button></a>
                                </td>-->
...
////                echo $total_amount1;exit;
        ...
        <!--                                <td class="text-left">   
                                ...
                                <td><?php echo date('d-m-Y', strtotime($row['date12'])); ?></td>
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        <form class="form form-validate" role="form" method="POST">
                                            <div class="row">
                                                <div class="col-sm-6  form-group">
                                                    <label for="expences type">&nbsp; &nbsp;Expenses Type <sup style="color:red;">*</sup></label>
                                                </div>
                                            </div>
                                            <div class="row">
                                                <div class="col-sm-6  form-group">
                                                    ...
                                                    <div class="col-sm-6 form-group ">
                                                        ...
                                                        <div class="form-group control-width-normal">
                                                            <div class="input-group date" id="demo-date">
                                                                <input type="text" class="form-control" required="true" autocomplete="off" name="date12" value="<?php
                                                                if (!$id) {
                                                                    echo "";
                                                                } else {
                                                                    echo $timestamp12;
