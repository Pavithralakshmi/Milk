...
$reduce_quantity = 0;
$prefix = $name = $gender = $cowtype = $breedtype = $date = $timestamp = $session = $total = $total_milk = $remark = $id = "";
...
if (isset($_POST['get_breed'])) {
    $sql34 = "Select * From `breedtype` WHERE `cowtype` = '$_POST[get_breed]' and `id` IN (select breedtype from cowreg WHERE gender='female' AND active!='no')  ";
    $result34 = mysqli_query($mysqli, $sql34);
    while ($row34 = mysqli_fetch_assoc($result34)) {
        $tid = $row34['id'];
        $tname = $row34['breedtype'];
        ?>
...
//            echo $sql;exit; 
        ...
        header("Location: Bulkmilkentry.php?msg=3");
        exit;
    } else {
        ...
        if (mysqli_num_rows($res)) {
            $msg = 1;
            header("Location: Bulkmilkentry.php?msg=1");
        } else {
            $sql = "INSERT INTO `dailymilk`(`type`,`cowtype`, `breedtype`, `date`,`session`, `remark`,`total`,`cby`, `cdate`, `cip`) VALUES ('bulk','$cowtype', '$breedtype', '$timestamp','$session','$remark','$total', '$user','$cdate','$_SERVER[REMOTE_ADDR]')";
//            echo $sql;exit;
            $result = mysqli_query($mysqli, $sql) or trigger_error("Query Failed! SQL: $sql - Error: " . mysqli_error($mysqli));
            header("Location: Bulkmilkentry.php?msg=2");
            exit;
        }
    }
}
if (isset($_GET['operation']) && $_GET['operation'] == 'delete') {
    ...
    header('Location: Bulkmilkentry.php?msg=' . $msg);
}
...
if (isset($_POST['tablechange'])) {
    ...
    <table id="datatable1" class="table diagnosis_list">
        ...
        <tbody class="ui-sortable" >
            <?php
            $i = 1;
            $sql343 = "SELECT `id`,`name`,`cowtype`, `breedtype`, `date`,`session`, `remark`,`total` from `dailymilk` where date(date) BETWEEN ('$startdate') AND ('$enddate') And `type`='bulk' ORDER BY `id` DESC";
            $result = mysqli_query($mysqli, $sql343);
            while ($row = mysqli_fetch_assoc($result)) {
                $id = $row['id'];
                $breedtype = $row['breedtype'];
                ?>
                <tr  id="<?php echo $row['id']; ?>"  >
                    <td><?php echo $i; ?></td>
                    <td class="text-left">   
                        <a href="Bulkmilkentry.php?id=<?php echo $id; ?>"><button type="button" class="btn ink-reaction btn-floating-action btn-info" data-toggle="tooltip" data-placement="top" data-original-title="Edit row"><i class="fa fa-pencil"></i></button></a>
                        <a href="Bulkmilkentry.php?id=<?php echo $id; ?>&operation=delete"><button  style="margin-bottom: 3px;" type="button" class="btn ink-reaction btn-floating-action btn-danger"    data-toggle="tooltip" onclick="return confirm('Are you sure to delete?')" data-placement="top" data-original-title="Delete row"><i class="fa fa-trash"></i></button></a>
                    </td>
                     <!--<td><?php // echo $get_cowtypee[$row['name']];                            ?></td>-->
...
<html lang="en">
    <head>
        <title>Milk Management- Bulkmilk</title>
                                ...
                                <div class="card">
                                    <form class="form form-validate" role="form" method="POST">
                                        <div class="card-head style-primary">
                                            <header>Bulk Milking Entry</header>
                                        </div>
                                        <div class="card-body">
                                            <div class="row">
                    ...
                    <div class=" col-md-12 col-sm-12">
                        <div class="card">
                            <div class="card-head style-primary" style="width:100%;">
                                <header>Bulk Milking Entry Details</header>
                            </div>
                            <div class="card-body">
                                <form class="form  form-validate" role="form" method="POST">
...
<script>
    ...
    $('input[name=gender]').change(function () {
        ...
        if (active === "daily") {
            ...
            $("#gender_active1").addClass("hide");
        } else {
            $("#gender_active").addClass("hide");
            $("#gender_active1").removeClass("hide");
        }
    });
</script>
<script>
